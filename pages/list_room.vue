...
<div v-else class="index">
	...
	<h1 hidden>{{ owner = $store.state.id }}</h1>
</div>
...
export default {
	...
	async created() {
		const { data } = await this.$axios.get(`${process.env.SERVER}/api/room?offset=${this.page}&owner=${this.owner}`)
		this.rooms = data
	},
	methods:{
	},
	watch:{
		async page() {
			const { data } = await this.$axios.get(`${process.env.SERVER}/api/room?offset=${parseInt(this.page)}&owner=${this.owner}`)
			this.rooms = data
		},
		async owner() {
			const { data } = await this.$axios.get(`${process.env.SERVER}/api/room?offset=${parseInt(this.page)}&owner=${this.owner}`)
